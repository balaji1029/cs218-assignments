...
struct Graph
{
	...
	pair<int, int> run(int threshold)
	{
		...
		int weight = 0;
		MST_Graph tree = MST_Graph(this->V);
		int count = 0;
		for (auto w : pq)
		{
			if (uf1.find(w[2]) != uf1.find(w[3]))
			{
				if (!w[1])
					uf0.Union(w[2], w[3]);
				else
		...
		while (weight <= threshold)
		{
			...
			for (int i = 0; i < rep.size(); i++)
			{
				if (rep[i] != 0)
				{
					...
					if (minimum_cost > (-pq[i][0] + pq[rep[i]][0]) && (pq[i][0] ^ pq[rep[i]][0]))
					{
			...
			cout << "First: " << red << " Second: " << blue << endl;
			cout << "Colors: " << pq[red][1] << " " << pq[blue][1] << endl;
			if (!haha)
				break;
